...
```
feat(search): add fuzzy matching for Korean compounds
fix(popup): prevent overflow on long definitions
refactor(content): extract hover handler into module
chore: bump dexie to 4.5
```
...
```bash
git checkout -b cm/feat/video-subtitles
git checkout -b wren/fix/korean-lemma-resolution
```
...
```
feat: add video subtitle support (by Wren)
fix: resolve Korean lemma lookup (by Lumen)
```
...
- **PascalCase** for classes and types (`SearchChineseUseCase`, `WordDefinitions`)
- **SCREAMING_SNAKE_CASE** for constants (`MAX_CHINESE_WORD_LENGTH`)
...
- Co-locate tests (`*.test.ts`) in `src/test/`
...
- Fail gracefully in content scripts (don't break the host page)
...
- **Build**: WXT (Vite-based), Manifest V3
- **UI**: React 18, TypeScript
- **Storage**: Dexie 4 (IndexedDB), chrome.storage
